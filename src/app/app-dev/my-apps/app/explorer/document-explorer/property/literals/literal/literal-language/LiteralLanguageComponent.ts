...
@Component( {
	selector: "literal-language",
	template: template,
	host: { "[class.error]": "!!input && !input.valid" },
	encapsulation: ViewEncapsulation.Emulated
} )

export default class LiteralLanguageComponent {
	...
	@Output() onLanguageSelected:EventEmitter<string> = new EventEmitter<string>();
	...
	ngOnInit():void {
		this.input = new Control( this.language, Validators.compose( [ Validators.required, this.validateSelectionInput.bind( this ) ] ) );
	}
	...
	initializeDropdown():void {
		this.$languages = $( this.element.nativeElement.querySelector( "select" ) );
		this.$languages.dropdown( {
			allowAdditions: false,
			onChange: this.onChangeType.bind( this )
		} );
		this.$languages.dropdown( "set selected", this.language );
	}

	onChangeType( value:string, text:string, choice:JQuery ):void {
		(<Control>this.input).updateValue( value === "empty" ? "" : value );
		this.onLanguageSelected.emit( value );
	}
