...
export class ResultsetTableComponent {
	...
	ngOnInit():void {
		...
		this.resultset.results.bindings = this.resultset.results.bindings.map( ( bindingObject ) => {
			let bindingArray:any = [];
			for( let varName of this.resultset.head.vars ) {
	...
	sortColumn( columnName:string ):void {
		...
		this.resultset.results.bindings.sort( ( bindingA, bindingB ) => {
