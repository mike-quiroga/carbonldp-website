...
@Service("s_IndirectContainerService")
@Scope(proxyMode = ScopedProxyMode.TARGET_CLASS, value = "request")
public class IndirectContainerService extends ContainerService {
	@Override
	public Container get(URIObject documentURIObject, List<RetrieveContainerPreference> preferences, String dataset) throws CarbonException {
		...
		String query = null;
		query = prepareIndirectContainerQuery(documentURI, preferences);
		...
		try {
			...
			if ( LOG.isErrorEnabled() ) {
				LOG.error("<< get() > The IndirectContainer couldn't be fetched.");
			}
			throw e;
		}
	...
	protected void addMembers(URIObject containerURIObject, Container container, List<? extends RDFSource> members, WriteTransactionTemplate template)
		...
		for(RDFSource member : members) {
			Statement icrStatement = member.getResource().getProperty(icrPredicate);
	...
	//@formatter:off
    protected String prepareIndirectContainerQuery(String documentURI, List<RetrieveContainerPreference> preferences) {
        StringBuffer query = new StringBuffer();
