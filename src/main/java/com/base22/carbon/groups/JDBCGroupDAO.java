...
@Service("groupDAO")
public class JDBCGroupDAO extends DAOJdbc implements GroupDAO {
	...
	public HashSet<Group> getImmediateGroupsOfAgent(UUID agentUUID) throws CarbonException {
		...
		try {
			//@formatter:off
			groups = template.execute(securityJDBCDataSource, new QueryTransactionCallback<HashSet<Group>>() {
				//@formatter:off
				@Override
				public StringBuilder prepareSQLQuery(StringBuilder sqlBuilder) {
					...
					sqlBuilder
						...
						.append(" WHERE ").append(AGENTS_GROUPS_TABLE).append(".").append(AgentDAOJdbc.EXTERNAL_ID_FIELD).append(" = UNHEX(?)")
					;
					//@formatter:on
					return sqlBuilder;
				}
	...
	public HashSet<Group> getAllGroupsOfAgent(UUID agentUUID) throws CarbonException {
		...
		for (final Group immediateGroup : immediateGroups) {
			...
			try {
				//@formatter:off
				parentGroups = template.execute(securityJDBCDataSource, new QueryTransactionCallback<HashSet<Group>>() {
					//@formatter:off
					@Override
					public StringBuilder prepareSQLQuery(StringBuilder sqlBuilder) {
						//@formatter:off
