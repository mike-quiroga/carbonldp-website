...
public class JDBCPrivilegeDAO extends DAOJdbc implements PrivilegeDAO {
			...
			privileges = template.execute(securityJDBCDataSource, new QueryTransactionCallback<HashSet<Privilege>>() {
				...
				public StringBuilder prepareSQLQuery(StringBuilder sqlBuilder) {
					...
					sqlBuilder
						...
						.append(" ON ")
							.append(AgentDAOJdbc.AGENTS_ROLES_TABLE).append(".").append(JDBCPlatformRoleDAO.EXTERNAL_ID_FIELD).append(" = ").append(PRIVILEGES_ROLES_TABLE).append(".").append(JDBCPlatformRoleDAO.EXTERNAL_ID_FIELD)
						.append(" INNER JOIN ")
							.append(TABLE)
						.append(" ON ")
